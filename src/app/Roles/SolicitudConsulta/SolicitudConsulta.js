...
descargar.addEventListener('click', async () => {
    ...
    const url = '../../assets/Archivos/PlantillaSolicitudRobots.xlsx';
    const element = document.createElement('a');

    element.href = url;

    element.download = 'PlantillaSolicitudRobots.xlsx';
    element.style.display = 'none';
...
async function guardarDatos(datosFinales) {
    ...
    const urlcompleta = urlBack.url + '/EstadosRobots/crear_Solicitud';
    try {
        fetch(urlcompleta, {
